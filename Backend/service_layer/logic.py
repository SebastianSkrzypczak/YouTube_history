...
def filter_videos_by(
    column_name: str, videos: pd.DataFrame, categories: list, exclude: bool
) -> pd.DataFrame:
...
def filter_videos_by_date(
    ...
    if not exclude:
        videos_filtered = videos[
            (videos[column_name] >= start_date) & (videos[column_name] <= end_date)
        ]
    else:
        videos_filtered = videos[
            (videos[column_name] < start_date) | (videos[column_name] > end_date)
...
def calculate_total_watch_time(
    history: pd.DataFrame,
    videos: pd.DataFrame,
    column_name: str = "",
    categories: list = [],
...
def show_most_viewed_channels(
    history: pd.DataFrame, videos: pd.DataFrame, count: int = 10
):
...
def time_activity_analysis(
    history: pd.DataFrame, start_date=pd.Timestamp.min, end_date=datetime.today()
) -> pd.DataFrame:
    ...
    hourly_count_df = pd.DataFrame(
        zip(list(hourly_count.index), list(hourly_count.values)),
        columns=["hour", "count of videos"],
    )
...
def statistics_in_time(
    ...
    for year in years:
        ...
        filtered_by_year = filter_videos_by_date(
            column_name="time",
        ...
        new_row = {
            "year": year,
            "title": most_viewed_videos["title"].values[0],
            "count": most_viewed_videos["count"].values[0],
