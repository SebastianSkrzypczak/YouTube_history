...
def filter_videos_by_date(column_name: str, start_date: datetime, end_date: datetime, videos: pd.DataFrame, exclude=False) -> pd.DataFrame:
    ...
    if not exclude:
        videos_filtered = videos[(videos[column_name] >= start_date) & (
            videos[column_name] <= end_date)]
    else:
        videos_filtered = videos[(videos[column_name] < start_date) | (
            videos[column_name] > end_date)]
    return videos_filtered
...
def any_analysis(analysis_by: str, history: pd.DataFrame, videos: pd.DataFrame, name='',
    ...
    if column_name != '':
        videos = filter_videos_by(
            column_name, videos, categories, exclude=exclude)
    if name == '':
        name = analysis_by
    merged = history.merge(videos, how='inner',
                           left_on='titleUrl', right_on='id')
    columns_to_drop = list(set(merged.columns.to_list()) - set([analysis_by]))
    merged = merged.drop(columns=columns_to_drop)
    merged = merged[analysis_by].value_counts(
    ).sort_values(ascending=False).head(count)
    merged_pd = pd.DataFrame(
        zip(list(merged.index), list(merged.values)), columns=[name, 'count'])
    return merged_pd
...
def calculate_total_watch_time(history: pd.DataFrame, videos: pd.DataFrame, column_name: str = "", categories: list = [], exclude: bool = True) -> float:
    ...
    """
    merged = history.merge(videos, left_on='titleUrl',
...
def show_most_viewed_videos(history: pd.DataFrame, videos: pd.DataFrame, count: int = 10, excluded_categories: list = []) -> pd.DataFrame:
    ...
    """
    most_viewed_DF = any_analysis(
        'id', history, videos, column_name='categoryId', categories=excluded_categories, count=count)
    merged = most_viewed_DF.merge(videos, how='inner')
    merged = merged.drop(columns=[
                         'publishedAt', 'channelId', 'categoryId', 'duration', 'viewCount', 'likeCount'])
    return merged
...
def time_activity_analysis(history: pd.DataFrame, start_date=pd.Timestamp.min, end_date=datetime.today()) -> pd.DataFrame:
    ...
    """
    history_filtered = filter_videos_by_date(
        'time', start_date, end_date, history)
    history_filtered['hours'] = history_filtered['time'].dt.hour
    hourly_count = history_filtered['hours'].value_counts().sort_index()
    hourly_count_df = pd.DataFrame(zip(list(hourly_count.index), list(
        hourly_count.values)), columns=['hour', 'count of videos'])
...
def statistics_in_time(history: pd.DataFrame, videos: pd.DataFrame, ):
    ...
    years_analytics = pd.DataFrame(
        columns=['year', 'title', 'count', 'total_watch_time'])
    for year in years:
        ...
        most_viewed_videos = show_most_viewed_videos(
            filtered_by_year, videos, count=1)
        total_watch_time = calculate_total_watch_time(filtered_by_year, videos)
        new_row = {'year': year, 'title': most_viewed_videos['title'].values[0],
                   'count': most_viewed_videos['count'].values[0], 'total_watch_time': total_watch_time}
        years_analytics = pd.concat(
            [years_analytics, pd.DataFrame([new_row])], ignore_index=True)
